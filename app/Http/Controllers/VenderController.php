...
class VenderController extends Controller
{
    ...
    public function venderVacation()
    {

       if (\Illuminate\Support\Facades\Auth::user()->can('manage vender vacation')){

            $venderVacations = UserVacation::where('vender_id',\Illuminate\Support\Facades\Auth::user()->vender_id)->get();
            return view('vender.indexVacation', compact('venderVacations'));
        }
        else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    ...
    public function createVenderVacation()
    {
        return view('vender.createVacation');
    }

    public function storeVenderVacation(Request $request)
    {

        UserVacation::create([
            'vender_id'=>\Illuminate\Support\Facades\Auth::user()->vender_id,
            'vender_name'=>\Illuminate\Support\Facades\Auth::user()->name,
            'fromH' => $request->fromH,
            'untilH' => $request->untilH,
            'firstDay' => $request->firstDay,
            'lastDay' => $request->lastDay
        ]);
        session()->flash('success', 'vacation submitted successfuly');
        return redirect(route('venderVacation.index'));
    }


    public function editVenderVacation($id)
    {

            $venderVacation = UserVacation::where('id', '=', $id)->get();
            return view('vender.createVacation', compact('venderVacation'));

    }

    public function updateVenderVacation(Request $request,$id)
    {
        $venderVacation=UserVacation::where('id','=',$id)->get();
        $venderVacation->update([
            'fromH' => $request->fromH,
            'untilH' => $request->untilH,
            'firstDay' => $request->firstDay,
            'lastDay' => $request->lastDay
    ...
    public function destroyVenderVacation($id)
    {
        $venderVacation=UserVacation::where('id',$id);
        $venderVacation->delete();
